    ...
    def face_recog(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbours,color,text,clf):
            ...
            for(x,y,w,h) in features:
                ...
                if confidence > 77:
                    cv2.putText(img,f"ID: {i}",(x,y-75),cv2.FONT_ITALIC ,1,(0,0,0),1)

                    cv2.putText(img,f"Roll: {r}",(x,y-55),cv2.FONT_ITALIC  ,1,(0,0,0),1)
                    cv2.putText(img,f"Name: {n}",(x,y-30),cv2.FONT_ITALIC ,1,(0,0,0),2)
                    cv2.putText(img,f"Department: {d}",(x,y-5),cv2.FONT_ITALIC ,1,(0,0,0),1)
